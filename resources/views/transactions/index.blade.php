            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="flex gap-4 mb-4">
                        <input type="text" class="form-input rounded-md shadow-sm w-full" placeholder="Search name...">
                            ...
                            <tbody class="text-gray-600 text-sm font-light">
                                @foreach ($transactions as $transaction)
                                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                                        ...
                                        <td class="py-3 px-6 text-left">{{ $transaction->rate }}</td>
                                        <td class="py-3 px-6 text-left">{{ \Carbon\Carbon::parse($transaction->date_paid)->format('d M Y') }}</td>
                                        <td class="py-3 px-6">
                                            <span class="px-3 py-1 text-xs text-white bg-gray-500 rounded-full">{{ $transaction->category }}</span>
                                        </td>
                                        @if(count($transaction->details) > 0)
                                            <td class="py-3 px-6">{{ $transaction->details[0]->name }}</td>
                                            <td class="py-3 px-6">Rp{{ number_format($transaction->details[0]->amount, 2) }}</td>
                                        @else
